...
function playDoneSound(kind){ // kind:'beats' 一声 | 'all' 快速两声
  if(!_snd.enabled) return;
  unlockAudio();
  if(!_snd.ctx || _snd.ctx.state !== 'running') return;
  if(kind === 'all'){ // 快速两声(上行 C6→E6)
    _sndBeep(1046.5, 0, 0.12); _sndBeep(1318.5, 0.15, 0.18);
  }else{               // 一声(叮咚 E5→A5)
    _sndBeep(659.3, 0, 0.1); _sndBeep(880.0, 0.12, 0.2);
  }
}
...
// v1.0.300：规划师卡「节拍表完成声音 + 音量」绑定（与设置弹窗同源同键，_cpsBound 防重复绑定）
function bindPlannerSoundTool(){
  ...
  if(vol && !vol._cpsBound){
    ...
    vol.addEventListener('change', ()=>{ playDoneSound('beats'); });   // 松开音量滑杆即试听一声
  }
}
...
3. 逐拍推进（核心）：必须严格按上方【当章微拍节奏】的每一拍，**逐拍单独成行**输出，格式为「拍名（约字数）：该拍具体发生什么」。字数取该拍自身标注的字数配比；内容要写出是谁、对谁、做了什么、结果/冲突/新信息，能落成画面和动作。禁止把各拍合并成整段散文、禁止省略或合并任何一拍、禁止只留一句骨架。
...
function viewStory(){
  ...
  <div class="flow-wrap">
    ...
    <section class="flow-sec" data-flow="3">
      <div class="flow-sec-head"><span class="fs-no">3</span><span class="fs-name">词典达人</span></div>
    ...
    <section class="flow-sec" data-flow="5">
      <div class="flow-sec-head"><span class="fs-no">5</span><span class="fs-name">词典充实</span></div>
  ...
  return `<div class="card cp-card">
    ...
    <div class="cp-body"${collapsed?' hidden':''}>
      ...
      ${hasPlans ? `<div class="cp-plans-tool">
          ...
          <button type="button" class="btn small cp-reader-btn" data-cp-reader title="像读书一样逐章阅读/切换/编辑各章节拍编排">📖 阅读节拍表</button>
        </div>`
        : ``}
      <!-- v1.0.300 完成声音 + 音量（内嵌规划师卡，紧邻节拍表）：让用户在做节拍表处即见「节拍表完成声音」，无需去设置弹窗 -->
      <div class="cp-sound-tool">
        <label class="cps-switch" title="节拍表完成响一声；一键四步全部完成响快速两声">
          <input id="cpsSoundDone" type="checkbox">
          <span class="cps-wrap"><i>🔔</i><b>节拍表完成声音</b></span>
        </label>
...
function bindBeatSheet(){
  ...
  bindPlannerSoundTool();   // v1.0.300 内嵌规划师卡的「节拍表完成声音 + 音量」绑定（与设置弹窗同源同键，_cpsBound 幂等）
}
...
function beatReaderChapters(){ return (state.outline && Array.isArray(state.outline.chapterPlans)) ? state.outline.chapterPlans : []; }
function readChTitle(i){
  const o = state.outline||{};
  const t = (o.chapters && o.chapters[i]) ? String(o.chapters[i].title||'') : '';
  return String(t).trim() ? String(t).trim() : `第${i+1}章`;
}
...
function dictMasterBlockHtml(){
  ...
  if(hasOut){
    ...
    return `<div class="card dm-card">
      <div class="dm-head dm-head-single">📖 词典达人 · 万物词典生成器<span class="muted" style="font-weight:400">（人物 ${r.nChar||0} · 地名 ${r.nPlace||0} · 专名 ${r.nProp||0}）</span></div>
...
async function genPlannerBeats(btn, opts){
  ...
  try{
    ...
    playDoneSound('beats');   // v1.0.299：节拍表（最耗时步）完成响一声
    return true;
  }catch(e){
...
async function genPlannerAll(btn){
  ...
  try{
    ...
    playDoneSound('all');   // v1.0.299：一键四步全部完成响快速两声
  }finally{
    finish();
    hideStopBtn();
...
function buildChapterUser(i, opt={}){
  ...
  if(plan && String(plan.beatsText||'').trim()){
    parts.push(`【L1 本章节拍编排（可依照的素材重心·纯文本）】\n${String(plan.beatsText).trim()}\n`);
  }
  ...
  if(i > 0){
    ...
    const prevBt = (Array.isArray(o.chapterPlans) && o.chapterPlans[i-1] && typeof o.chapterPlans[i-1].beatsText==='string' && o.chapterPlans[i-1].beatsText.trim()) ? o.chapterPlans[i-1].beatsText.trim() : '';
    if(prevBt){
