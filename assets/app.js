...
(function initSoundUI(){
  ...
  const bind = ()=>{ const el = document.getElementById('cfgSoundDone'); if(!el) return;
    el.addEventListener('change', ()=>{ const on = !!el.checked; try{ localStorage.setItem(SND_KEY, on?'1':'0'); }catch(e){} _snd.enabled = on; });
    const vl = document.getElementById('cfgSoundVol'), lb = document.getElementById('cfgSoundVolLabel');
    if(vl){
      vl.addEventListener('input', ()=>{
        const pct = Math.min(100, Math.max(0, +vl.value||0)); _snd.vol = pct/100;
...
5. 出场实体：本章要落到文中的人名/地名/专名（优先取自设定词典；本章确需新增的配角/路人/地名/专名一并纳入，并在底部【本章实体清单】统一登记）。
6. 埋设伏笔：本章要在文中悄悄埋下什么线索（可留空，有则写，措辞直白不绕弯）。
7. 收束设计：本章以何种落地/悬置方式收尾并指向下一章。
【输出格式（严格纯文本，不是 JSON）】
对输入中的每一章各输出一个章节块，格式严格为（前后各空一行）：
===== 第N章 =====
（该章编排正文：承接点 / 场景链 / 逐拍推进 / 情绪弧 / 出场实体 / 埋设伏笔 / 收束。）
